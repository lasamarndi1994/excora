  ...
  <div class="board-page" @click="closeAllMenus">
    ...
    <div class="board-bar">
      <div class="bar-left">
        ...
        <!-- Assignee avatars -->
        <div class="avatars">
          <div v-for="(user, i) in store.users.slice(0, 5)" :key="user.id" class="av-wrap" :title="user.name"
            :style="{ marginLeft: i > 0 ? '-6px' : '0', zIndex: 5 - i }"
            :class="{ 'av-active': assigneeFilter.includes(user.id) }" @click.stop="toggleAssigneeFilter(user.id)">
        ...
        <div class="dd-wrap" ref="filterWrapRef">
          ...
          <div v-if="openDd === 'filter'" class="filter-panel" @click.stop>
            <div class="fp-sidebar">
              <div v-for="cat in filterCategories" :key="cat.value" class="fp-cat"
                :class="{ active: activeFilterCat === cat.value }" @click="activeFilterCat = cat.value">
                {{ cat.label }}
              </div>
            </div>
            <div class="fp-body">
              ...
              <template v-if="activeFilterCat === 'assignee'">
                ...
                <div v-for="u in store.users" :key="u.id" class="fp-check-row" @click.stop="toggleAssigneeFilter(u.id)">
                  <span class="fp-checkbox" :class="{ checked: assigneeFilter.includes(u.id) }">
                    <svg v-if="assigneeFilter.includes(u.id)" width="9" height="9" viewBox="0 0 24 24" fill="none">
              ...
              <template v-else-if="activeFilterCat === 'type'">
                ...
                <div v-for="t in issueTypes" :key="t" class="fp-check-row" @click.stop="toggleTypeFilter(t)">
                  <span class="fp-checkbox" :class="{ checked: typeFilter.includes(t) }">
                    <svg v-if="typeFilter.includes(t)" width="9" height="9" viewBox="0 0 24 24" fill="none">
                      <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                    </svg>
                  </span>
              ...
              <template v-else-if="activeFilterCat === 'priority'">
                ...
                <div v-for="p in ['High', 'Medium', 'Low']" :key="p" class="fp-check-row"
                  @click.stop="togglePriorityFilter(p)">
                  <span class="fp-checkbox" :class="{ checked: priorityFilter.includes(p) }">
                    <svg v-if="priorityFilter.includes(p)" width="9" height="9" viewBox="0 0 24 24" fill="none">
                      <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                    </svg>
                  </span>
              ...
              <template v-else-if="activeFilterCat === 'epic'">
                ...
                <div v-for="e in store.epics" :key="e.id" class="fp-check-row" @click.stop="toggleEpicFilter(e.id)">
                  <span class="fp-checkbox" :class="{ checked: epicFilter.includes(e.id) }">
                    <svg v-if="epicFilter.includes(e.id)" width="9" height="9" viewBox="0 0 24 24" fill="none">
                      <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                    </svg>
                  </span>
              ...
              <template v-else>
                ...
                <div v-for="s in store.sprints" :key="s.id" class="fp-check-row" @click.stop="toggleSprintFilter(s.id)">
                  <span class="fp-checkbox" :class="{ checked: sprintFilter.includes(s.id) }">
                    <svg v-if="sprintFilter.includes(s.id)" width="9" height="9" viewBox="0 0 24 24" fill="none">
                      <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                    </svg>
                  </span>
                  <span>{{ s.name }}</span>
        ...
        <template v-if="hasFilters">
          ...
          <span v-for="eid in epicFilter" :key="'e' + eid" class="fchip fchip--epic">
            {{store.epics.find(e => e.id === eid)?.name}}<button class="fchip-x"
              @click.stop="toggleEpicFilter(eid)">×</button>
          </span>
      ...
      <div class="bar-right">
        ...
        <div class="dd-wrap">
          ...
          <div v-if="openDd === 'group'" class="dd-panel dd-panel-right" @click.stop>
            ...
            <div v-for="g in ['None', 'Assignee', 'Priority', 'Epic']" :key="g" class="dd-radio-row"
              :class="{ active: groupBy === g }" @click="groupBy = g; openDd = null">
        ...
        <div class="dd-wrap">
          ...
          <div v-if="openDd === 'sort'" class="dd-panel dd-panel-right" @click.stop style="min-width:180px">
            ...
            <div v-for="s in sortOptions" :key="s.value" class="dd-radio-row" :class="{ active: sortBy === s.value }"
              @click="sortBy = s.value; openDd = null">
          ...
          <div v-if="openDd === 'more'" class="dd-panel dd-panel-right" @click.stop style="min-width:200px">
            ...
            <div class="dd-check-row" @click.stop="showEpicLabels = !showEpicLabels">
              <span class="dd-checkbox" :class="{ checked: showEpicLabels }">
                <svg v-if="showEpicLabels" width="9" height="9" viewBox="0 0 24 24" fill="none">
                  <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                </svg>
              </span>
              Show epic labels
            ...
            <div class="dd-check-row" @click.stop="showPriorityBadge = !showPriorityBadge">
              <span class="dd-checkbox" :class="{ checked: showPriorityBadge }">
                <svg v-if="showPriorityBadge" width="9" height="9" viewBox="0 0 24 24" fill="none">
                  <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                </svg>
              </span>
              Show priority badge
            ...
            <div class="dd-check-row" @click.stop="showSubtaskProgress = !showSubtaskProgress">
              <span class="dd-checkbox" :class="{ checked: showSubtaskProgress }">
                <svg v-if="showSubtaskProgress" width="9" height="9" viewBox="0 0 24 24" fill="none">
                  <path d="M5 13l4 4L19 7" stroke="#fff" stroke-width="3" stroke-linecap="round" />
                </svg>
              </span>
          ...
          <div v-for="col in boardColumns" :key="col.id" class="board-col">
              ...
              <div class="col-hd-right">
                ...
                <div style="position:relative">
                  ...
                  <div v-if="colMenuOpen === col.id" class="col-dropdown" @click.stop>
                    ...
                    <button v-if="!col.isDefault" class="cdd-item cdd-item--danger"
                      @click="removeColumn(col.id); colMenuOpen = null">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
            ...
            <div v-if="renamingColId === col.id" class="col-rename-wrap" @click.stop>
              <input class="col-rename-input" v-model="renameValue" ref="renameInputRef"
                @keydown.enter="confirmRename(col)" @keydown.esc="renamingColId = null" @blur="confirmRename(col)" />
            </div>

            <!-- Drop zone -->
            <div class="col-body" :class="{ 'col-body--over': dragOver === col.id }"
              @dragover.prevent="onColDragOver(col.id)" @dragleave="onColDragLeave(col.id)"
              ...
              <template v-for="issue in getColIssues(col)" :key="issue.id">
                ...
                <div class="issue-card" :class="{ 'issue-card--dragging': dragId === issue.id }" draggable="true"
                  @dragstart="onDragStart(issue, col.id, $event)" @dragend="onDragEnd"
                  ...
                  <div class="ic-footer">
                    ...
                    <div class="ic-assignees">
                      <img v-for="(a, i) in issue.assignees.slice(0, 2)" :key="a.id" :src="a.avatar" :title="a.name"
                        class="ic-av" :style="{ marginLeft: i > 0 ? '-5px' : '0', zIndex: 2 - i }" />
                    </div>
                  </div>
                </div>
              </template>
              ...
              <div v-if="inlineCreateCol === col.id" class="inline-create" @click.stop>
                <input class="ic-input" v-model="inlineCreateText" ref="inlineInputRef" placeholder="Issue summary…"
                  @keydown.enter="confirmInlineCreate(col)" @keydown.esc="inlineCreateCol = null" />
          ...
          <div class="swimlane-board">
            ...
            <div v-for="group in groupedView" :key="group.key" class="swimlane-row">
              <div class="swimlane-row-label" @click="toggleSwimlane(group.key)">
                <svg class="sl-arrow" :class="{ collapsed: collapsedSwimlanes.includes(group.key) }" width="11"
                  height="11" viewBox="0 0 24 24" fill="none">
                  <polyline points="6 9 12 15 18 9" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" />
                </svg>
              ...
              <template v-if="!collapsedSwimlanes.includes(group.key)">
                <div v-for="col in boardColumns" :key="col.id" class="swimlane-col-body"
                  ...
                  @dragover.prevent="onColDragOver(col.id + group.key)" @dragleave="onColDragLeave(col.id + group.key)"
                  @drop.prevent="onDrop(col.id, $event)">
                  <template v-for="issue in getSwimlaneColIssues(group, col)" :key="issue.id">
                    <div v-if="dropTarget.colId === col.id + group.key && dropTarget.beforeId === issue.id"
                      class="drop-line"></div>
                    <div class="issue-card" :class="{ 'issue-card--dragging': dragId === issue.id }" draggable="true"
                      @dragstart="onDragStart(issue, col.id, $event)" @dragend="onDragEnd"
                      ...
                      <div class="ic-footer">
                        ...
                        <div class="ic-assignees">
                          <img v-for="(a, i) in issue.assignees.slice(0, 2)" :key="a.id" :src="a.avatar" :title="a.name"
                            class="ic-av" :style="{ marginLeft: i > 0 ? '-5px' : '0' }" />
                        </div>
                      </div>
                    </div>
                  </template>
                  <div v-if="dropTarget.colId === col.id + group.key && dropTarget.beforeId === null" class="drop-line">
                  </div>
                  <button class="create-btn" @click.stop="openCreateIssue(col.id)">
      ...
      <div v-if="showCompleteDialog" class="modal-overlay" @click.self="showCompleteDialog = false">
        <div class="modal-box" @click.stop>
          ...
          <div class="modal-body">
            ...
            <p class="modal-sub">{{ openIssueCount }} open {{ openIssueCount === 1 ? 'issue' : 'issues' }} will be moved
              to
              the backlog.</p>
...
.fp-sprint-status.sprint-active {
  background: #dcfce7;
  color: #16a34a;
}

.fp-sprint-status.sprint-completed {
  background: #f1f5f9;
  color: #94a3b8;
}
