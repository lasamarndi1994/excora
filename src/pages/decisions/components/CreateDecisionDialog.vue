    ...
    <v-dialog :model-value="modelValue" @update:model-value="onUpdateModelValue" max-width="600" scrollable scroll-strategy="none">
        <v-card class="rounded-lg">
            <v-card-title class="d-flex justify-space-between align-center px-6 py-4 border-b">
                <span class="text-h6 font-weight-bold">Create Decision</span>
            ...
            <v-card-actions class="px-6 py-4 border-t d-flex justify-end gap-3 bg-grey-lighten-5">
                ...
                <v-btn color="primary" variant="flat" class="text-none px-6 font-weight-medium" @click="save">Create
                    Decision</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
...
const emit = defineEmits<{
    ...
    (e: 'save', data: Omit<Decision, 'id' | 'createdBy'>): void
}>()
...
const save = () => {
    ...
    emit('save', {
        ...
        linkedWork: formData.value.linkedWork,
        optionsConsidered: formData.value.optionsConsidered,
        decisionMade: formData.value.decisionMade,
