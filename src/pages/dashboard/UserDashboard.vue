  ...
  <div class="dash">
    ...
    <div class="stat-row">
      <div v-for="stat in quickStats" :key="stat.label"
        class="stat-card" :style="{ '--accent': stat.accent }">
          ...
          <div class="sprint-overview">
            <div class="sprint-donut-wrap">
              <svg width="80" height="80" viewBox="0 0 80 80">
                ...
                <circle cx="40" cy="40" r="32" fill="none" stroke="#4f46e5" stroke-width="10"
                  stroke-dasharray="201" :stroke-dashoffset="201 - (201 * sprintPct / 100)"
                  stroke-linecap="round" transform="rotate(-90 40 40)"/>
                <text x="40" y="44" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">{{ sprintPct }}%</text>
              </svg>
            </div>
          ...
          <div class="sprint-breakdown">
            <div v-for="b in sprintBreakdown" :key="b.label" class="sb-row">
              ...
              <div class="sb-bar-wrap">
                <div class="sb-bar">
                  <div class="sb-fill" :style="{ width: (b.count / sprintTotal * 100) + '%', background: b.color }"></div>
                </div>
        ...
        <div class="panel">
          <div class="panel-head">
            ...
            <div class="chart-tabs">
              <button v-for="t in ['Week','Month']" :key="t"
                class="chart-tab" :class="{ active: chartTab === t }"
          ...
          <div class="chart-wrap">
            <svg width="100%" height="120" viewBox="0 0 420 120" preserveAspectRatio="none">
              ...
              <line v-for="y in [20,50,80,110]" :key="y" x1="0" :y1="y" x2="420" :y2="y" stroke="#f1f5f9" stroke-width="1"/>
              <polygon :points="chartAreaPts" fill="url(#chartGrad)"/>
              <polyline :points="chartLinePts" fill="none" stroke="#4f46e5" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>
              <circle v-for="(p, i) in chartPoints" :key="i" :cx="p.x" :cy="p.y" r="3.5" fill="#4f46e5" stroke="white" stroke-width="1.5"/>
            </svg>
      ...
      <div class="right-col">
        ...
        <div class="panel">
          ...
          <div class="schedule-list">
            <div v-for="ev in todaySchedule" :key="ev.id" class="schedule-item"
...
const chartAreaPts = computed(() => {
  ...
  return `${pts.map(p => `${p.x},${p.y}`).join(' ')} ${last.x},120 ${pts[0].x},120`
})
...
.dash-sub { font-size: 12.5px; color: #64748b; margin: 0; }
...
.stat-card:hover .stat-glow { opacity: .08; transform: scale(1.4); }
.stat-card:hover .stat-icon-bg { transform: rotate(-8deg) scale(1.1); }
.stat-card:hover .stat-val { transform: scale(1.05); }
.stat-card:hover .stat-bar-fill { filter: brightness(1.1); }
...
.stat-card:hover .stat-sparkle { opacity: 1; animation: sparkle-pop .4s ease forwards; }
@keyframes sparkle-pop {
  0%   { transform: scale(0); opacity: 1; }
  60%  { transform: scale(1.6); opacity: .8; }
  100% { transform: scale(1); opacity: 1; }
}
.stat-info { flex: 1; }
.stat-label { font-size: 11.5px; color: #64748b; margin-bottom: 4px; }
.stat-val {
  font-size: 30px; font-weight: 700; color: #0f172a;
  line-height: 1; transition: transform .2s; display: inline-block;
}
.stat-bar { height: 4px; background: #f1f5f9; border-radius: 99px; margin-bottom: 10px; overflow: hidden; }
.stat-bar-fill { height: 100%; border-radius: 99px; transition: width .6s cubic-bezier(.4,0,.2,1), filter .2s; }
.stat-footer { display: flex; align-items: center; justify-content: space-between; }
.stat-trend { display: flex; align-items: center; gap: 3px; font-size: 11px; font-weight: 600; }
.trend-up   { color: #10b981; }
.trend-down { color: #ef4444; }
.stat-period { font-size: 10.5px; color: #94a3b8; }
.stat-glow {
...
@media (max-width: 640px) { .dash { padding: 14px; } .dash-header { flex-wrap: wrap; gap: 10px; } }
...
.panel-icon-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
...
.panel-action-btn:hover { opacity: .7; }
...
.task-cb { width: 14px; height: 14px; accent-color: #4f46e5; cursor: pointer; }
...
.task-meta { display: flex; align-items: center; gap: 5px; font-size: 11px; }
.meta-due { display: flex; align-items: center; gap: 3px; color: #64748b; }
.due-today    { color: #ef4444; font-weight: 600; }
.due-tomorrow { color: #f59e0b; font-weight: 600; }
.meta-dot { width: 3px; height: 3px; border-radius: 50%; background: #cbd5e1; }
.meta-proj { display: flex; align-items: center; gap: 3px; color: #94a3b8; }
.priority-pill {
  font-size: 11px; font-weight: 600; padding: 2px 9px;
  border-radius: 20px; white-space: nowrap; flex-shrink: 0;
  transition: transform .15s;
}
.task-row:hover .priority-pill { transform: scale(1.05); }
...
.row-chevron { color: #cbd5e1; flex-shrink: 0; transition: transform .15s, color .15s; }
.task-row:hover .row-chevron { transform: translateX(3px); color: #94a3b8; }

/* ── Sprint ── */
.sprint-label { font-size: 11.5px; font-weight: 600; color: #8b5cf6; background: #f5f3ff; padding: 2px 9px; border-radius: 20px; }
.sprint-overview { display: flex; align-items: center; gap: 20px; padding: 16px 16px 12px; }
.sprint-donut-wrap { flex-shrink: 0; }
.sprint-stats { display: flex; flex-direction: column; gap: 8px; flex: 1; }
.sprint-stat-item { display: flex; align-items: center; justify-content: space-between; }
.sprint-stat-val { font-size: 14px; font-weight: 700; }
.sprint-stat-label { font-size: 11px; color: #94a3b8; }
.sprint-breakdown { padding: 0 16px 14px; display: flex; flex-direction: column; gap: 8px; }
.sb-row { display: flex; align-items: center; gap: 10px; }
.sb-label-wrap { display: flex; align-items: center; gap: 6px; width: 90px; flex-shrink: 0; }
.sb-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.sb-label { font-size: 11.5px; color: #475569; }
.sb-bar-wrap { display: flex; align-items: center; gap: 8px; flex: 1; }
.sb-bar { flex: 1; height: 6px; background: #f1f5f9; border-radius: 99px; overflow: hidden; }
.sb-fill { height: 100%; border-radius: 99px; transition: width .5s; }
.sb-count { font-size: 11.5px; font-weight: 600; color: #64748b; width: 20px; text-align: right; }

/* ── Productivity chart ── */
.chart-tabs { display: flex; border: 1px solid #e2e8f0; border-radius: 7px; overflow: hidden; }
.chart-tab {
  padding: 4px 10px; font-size: 11.5px; font-weight: 500; color: #64748b;
  background: #fff; border: none; cursor: pointer; font-family: 'Inter', sans-serif;
  transition: background .12s, color .12s;
}
.chart-tab.active { background: #eef2ff; color: #4f46e5; font-weight: 600; }
.chart-wrap { padding: 12px 16px 4px; }
.chart-labels {
  display: flex; justify-content: space-between;
  padding: 4px 0 8px; font-size: 10.5px; color: #94a3b8;
}

/* ── Schedule ── */
.schedule-date { font-size: 11.5px; color: #94a3b8; }
...
.ev-time-col { width: 58px; flex-shrink: 0; }
.ev-time { font-size: 11px; color: #94a3b8; font-weight: 500; }
.ev-bar { width: 3px; height: 32px; border-radius: 99px; flex-shrink: 0; }
.ev-body { flex: 1; min-width: 0; }
.ev-title { font-size: 12.5px; font-weight: 600; color: #0f172a; margin-bottom: 2px; }
.ev-meta { font-size: 11px; color: #94a3b8; display: flex; align-items: center; gap: 3px; }
.ev-type-badge { font-size: 10.5px; font-weight: 600; padding: 2px 8px; border-radius: 20px; white-space: nowrap; flex-shrink: 0; }
...
.act-dot-wrap { display: flex; flex-direction: column; align-items: center; flex-shrink: 0; padding-top: 3px; }
.act-dot {
  width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0;
  box-shadow: 0 0 0 3px #fff; transition: transform .2s;
}
.act-row:hover .act-dot { transform: scale(1.3); }
.act-line { width: 1px; flex: 1; min-height: 18px; background: #e2e8f0; margin-top: 4px; }
.act-body { flex: 1; }
.act-text { font-size: 12px; line-height: 1.5; color: #1e293b; }
.act-user   { font-weight: 600; }
.act-action { color: #64748b; }
.act-link   { color: #4f46e5; text-decoration: none; font-weight: 500; }
.act-link:hover { text-decoration: underline; }
.act-time   { font-size: 11px; color: #94a3b8; margin-top: 2px; }
.view-all-wrap { padding: 10px 12px; border-top: 1px solid #f1f5f9; }
.view-all-btn {
...
.view-all-btn:hover { background: #eef2ff; border-color: #c7d2fe; color: #4f46e5; }

/* ── Team workload ── */
.team-list { padding: 8px 0; }
.team-row {
  display: flex; align-items: center; gap: 10px;
  padding: 9px 16px; transition: background .12s; cursor: default;
}
...
.team-name { font-size: 12px; font-weight: 600; color: #0f172a; margin-bottom: 4px; }
.team-bar-wrap { display: flex; align-items: center; gap: 6px; }
.team-bar { flex: 1; height: 5px; background: #f1f5f9; border-radius: 99px; overflow: hidden; }
.team-fill { height: 100%; border-radius: 99px; transition: width .5s; }
.team-pct { font-size: 10.5px; font-weight: 600; color: #64748b; width: 28px; text-align: right; }
.team-tasks-count { font-size: 11px; color: #94a3b8; white-space: nowrap; flex-shrink: 0; }
